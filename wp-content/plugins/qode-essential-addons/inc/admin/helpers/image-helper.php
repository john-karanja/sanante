...
if ( ! function_exists( 'qode_essential_addons_framework_get_attachment_id_from_url' ) ) {
	/**
	 ...
	 * @return null|string
	 */
	function qode_essential_addons_framework_get_attachment_id_from_url( $attachment_url ) {
		global $wpdb;
		$attachment_id = '';
...
if ( ! function_exists( 'qode_essential_addons_framework_get_attachment_thumb_url' ) ) {
	/**
	 ...
	 * @return bool|string
	 *
	 * @see qode_essential_addons_framework_get_attachment_id_from_url()
	 */
	function qode_essential_addons_framework_get_attachment_thumb_url( $attachment_url ) {
		$attachment_id = qode_essential_addons_framework_get_attachment_id_from_url( $attachment_url );

		if ( ! empty( $attachment_id ) ) {
			return qode_essential_addons_get_attachment_thumb_url( $attachment_id );
		} else {
...
if ( ! function_exists( 'qode_essential_addons_framework_get_image_html_from_src' ) ) {
	/**
	 ...
	 * @return string
	 */
	function qode_essential_addons_framework_get_image_html_from_src( $url, $attr = array() ) {
		$html = '';
...
if ( ! function_exists( 'qode_essential_addons_framework_resize_image' ) ) {
	/**
	 ...
	 * @see qode_essential_addons_get_attachment_url()
	 * @see wp_get_image_editor()
	 */
	function qode_essential_addons_framework_resize_image( $attachment, $width = null, $height = null, $crop = true ) {
		$return_array = array();
		...
		if ( ! empty( $attachment ) ) {
			if ( is_int( $attachment ) ) {
				...
				$attachment_id = qode_essential_addons_framework_get_attachment_id_from_url( $attachment );
			}
			...
			if ( ! empty( $attachment_id ) && ( isset( $width ) && isset( $height ) ) ) {
				...
				//get attachment url
				$img_url = qode_essential_addons_get_attachment_url( $attachment_id );
...
if ( ! function_exists( 'qode_essential_addons_framework_generate_thumbnail' ) ) {
	/**
	 * Generates thumbnail img tag. It calls qode_essential_addons_framework_resize_image function for resizing image
	 *
	 ...
	 *
	 * @see qode_essential_addons_framework_resize_image()
	 * @see qode_essential_addons_framework_get_attachment_id_from_url()
	 */
	function qode_essential_addons_framework_generate_thumbnail( $attachment, $width = null, $height = null, $crop = true ) {
		if ( ! empty( $attachment ) ) {
			if ( is_int( $attachment ) ) {
				$attachment_id = $attachment;
			} else {
			...
			$img_info = qode_essential_addons_framework_resize_image( $attachment_id, $width, $height, $crop );
			...
			if ( is_array( $img_info ) && count( $img_info ) ) {
				...
				return qode_essential_addons_framework_get_image_html_from_src( $url, $attr );
			}
		}
